...
class Generator:
    def __init__(self, file_path, ft_hi, ft_en, labels2Idx: dict, emotions_dictionary: str = None):
        '''
        ...
        self.weight_file_hi = '/home1/zishan/WordEmbeddings/FastText/muse_weights/best_mapping_hi.pth'
        self.weight_file_bn = '/home1/zishan/WordEmbeddings/FastText/muse_weights/best_mapping_bn.pth'
        self.weight_hi = torch.from_numpy(torch.load(weight_file_hi)).cuda()

        self.mapping_hi = nn.Linear(300, 300, bias=False).cuda()
        self.mapping_hi.weight.data.copy_(weight_hi)
    ...
    def process_features(self, data, seq_len, ft, language):
        ...
        for j in range(min(seq_len, len(tweet))):
            embedding_vector = ft.get_word_vector(tweet[j].lower())
            embedding_vector = torch.from_numpy(embedding_vector)
            if language ==0: # hindi
                transformed_vector = self.mapping_hi(embedding_vector)
                # transformed_vector = transformed_vector.cpu().detach().numpy()
            elif language ==1: #english
                transformed_vector = embedding_vector
            features[j] = transformed_vector
        return features
                ...
                try:
                    ...
                    if language == 1: #english
                        batch_features_ft[i] = self.process_features(
                            data[0].loc[self.count], seq_len, self.ft_en, language)
                    elif language ==0 :
                          batch_features_ft[i] = self.process_features(
                            data[0].loc[self.count], seq_len, self.ft_hi, language)
