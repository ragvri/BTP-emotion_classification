...
class Generator:
    ...
    def generate(self, batch_size: int, seq_len:int = 75):
        ...
        print(self.count)
        while True:			
            batch_features_ft = np.zeros((batch_size,seq_len,300),dtype=np.float32)
            batch_class = np.zeros((batch_size,1),dtype=int)
            batch_intensity = np.zeros((batch_size, 1), dtype = float)
            
            for i in range(batch_size):
                batch_features_ft[i] = self.process_features(data[0].loc[self.count],seq_len)
                batch_class[i][0]  = labels2Idx[data[1].loc[self.count]]
                batch_intensity[i][0]  = data[2].loc[self.count]
                self.count = (self.count + 1) % data_len
            
            yield ([batch_features_ft, 1], [batch_class, batch_intensity])
if __name__ == "__main__":
    ...
    train_generator = Generator('/home1/zishan/raghav/emotion/MultilingualMultitask/dataset/news_hindi/data_with_intensity_hindi.txt', ft , labels2Idx)
